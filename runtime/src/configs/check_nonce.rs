...
/// Operation to perform from `validate` to `prepare` in [`frame_system::CheckNonce`] transaction
/// extension.
    ...
    <T::RuntimeCall as Dispatchable>::RuntimeOrigin: AsSystemOriginSigner<T::AccountId> + Clone,
{
    ...
    fn weight(&self, _: &T::RuntimeCall) -> Weight {
        <T::ExtensionsWeightInfo as ExtensionsWeightInfo>::check_nonce()
    }
    ...
    fn prepare(
        ...
        nonce += T::Nonce::one();
        Account::<T>::mutate(who, |account| account.nonce = nonce);
        Ok(Pre::NonceChecked)
    }
